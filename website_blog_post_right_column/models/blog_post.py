...
class BlogPost(models.Model):
    ...
    def get_most_read(self, carousel=False):
        ...
        posts = self.env['blog.post'].sudo().search([('blog_id','=',self.blog_id.id),('website_published','=',True)], order='visits desc',limit=max_posts)
        if posts:
            post_most_read=[]
            for p in posts:
                if p.id!=self.id:
                    if len(p['subtitle'])>35:
                        ...
                        subt = p['subtitle'] 
                    published = p.published_date.strftime('%B %Y')
                    published_long = p.published_date.strftime('%B %d %Y %H:%M hrs.')
            ...
            for p in posts:
                if p.id!=self.id:
                    if len(p['subtitle'])>35:
                        ...
                        subt = p['subtitle']                     
                    published = p.published_date.strftime('%B %d %Y')
                    published_long = p.published_date.strftime('%B %d %Y %H:%M')
                    post_url = "/blog/%s/post/%s" % (slug(p.blog_id), slug(p))
                    img_path = '/website_blog/static/src/img/bp_'
                    author = p.author_id.name
                    cover = self.get_cover()
    ...
    def create_images(self):
        ...
        if img_url != 'none':
            ...
            #image_path =  '/opt/odoo/server/addons/website_blog/static/src/img/'
