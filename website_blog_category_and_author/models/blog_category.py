...
class BlogCategory(models.Model):
    ...
    name = fields.Char(required=True, help='Canonical', translate=True)
    color = fields.Char(string='Color', size=7,  help="Choose your color", default="#FFFFFF")
    ...
    def _check_parent_id_child_id(self):
        ...
        for record in self:
            ...
            if record.parent_id.blog_id:
                same_parent = record.blog_id != record.parent_id.blog_id
            else:
                same_parent = False                
            conditions = (len(matches), same_parent)
            if any(conditions):
